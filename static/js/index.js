...
odoo.define('gpsmap.fleet', function (require) {
    ...
    var class_gpsmap = AbstractAction.extend({
        ...
        {
            if(local.vehicles != null && local.vehicles.length > 0)
            {
                this.$el.html(QWeb.render(this.template, {'vehicles': local.vehicles}));
            }
        },
        ...
        async CreateMap(iZoom,iMap,coordinates,object)
        {
            ...
            if($("div#"+object).length>0 && $("div#"+object).html()  ==  "")
            {
                ...
                this.obj_map = await new google.maps.Map(mapC.get(0), mapOptions);
                this.geocoder = await new google.maps.Geocoder();
                var trafficLayer = await new google.maps.TrafficLayer();
                trafficLayer.setMap(this.obj_map);

                this.geofence = await new google.maps.Polygon();
                this.gMEvent = await google.maps.event;
            }
            else
            {
                setTimeout(function()
